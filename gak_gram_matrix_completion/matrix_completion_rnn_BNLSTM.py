...
from kerassupervisedrnn import BNLSTM
...
def create_base_network(input_shape, mask_value):
    ...
    seq.add(BNLSTM(units=1000, kernel_regularizer=l2(0.01), recurrent_regularizer=l2(0.01),
                 dropout=0.5, implementation=1, return_sequences=False))
    seq.add(Dropout(0.5))
...
def main():
    ...
    html_out_rnn = filename.replace(".mat", "_loss" + str(incomplete_percentage) + "_RNN_BNLSTM.html")
    mat_out_rnn  = filename.replace(".mat", "_loss" + str(incomplete_percentage) + "_RNN_BNLSTM.mat")
    hdf5_out_rnn  = filename.replace(".mat", "_hdf5" + str(incomplete_percentage) + "_RNN_BNLSTM.hdf5")

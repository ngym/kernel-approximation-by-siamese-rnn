...
if __name__ == "__main__":
    ...
    for dataset_type in list(cnf['dataset'].keys()):
        experiments_dir = os.path.join(root_dir, "USE_CASE_SOFTIMPUTE_COMPLETION_1_VALIDATION")
        if dataset_type in {"UCIauslan", "UCItctodd"}:
            pkl_file_path = os.path.join(experiments_dir,
                                         "original_gram_files/gram_UCIauslan_sigma12.000.pkl")
            #sample_dir = os.path.join(experiments_dir, "datasets/UCIauslan/all")
            kfold = KFold_UCIauslan
        elif dataset_type == "UCIcharacter":
            pkl_file_path = os.path.join(experiments_dir,
                                         "original_gram_files/gram_UCIcharacter_sigma20.000.pkl")
            #sample_dir = os.path.join(experiments_dir, "datasets/UCIcharacter")
            kfold = KFold
        elif dataset_type in {"6DMGupperChar", "6DMG", "upperChar"}:
            pkl_file_path = os.path.join(experiments_dir,
                                         "original_gram_files/gram_upperChar_all_sigma20.000_t1-t3.pkl")
        ...
        for fold in folds:
            if "data_attribute_type" in list(cnf_d.keys()): # 6DMG
                settings = [(dat, os.path.join(dat),
                             Template(output_filename_format_).substitute(data_attribute_type=dat))
                            for dat in cnf_d['data_attribute_type']]
            else:
                settings = [(None, "", output_filename_format_)]

            for dat, ex_dir_, output_filename_format in settings:
